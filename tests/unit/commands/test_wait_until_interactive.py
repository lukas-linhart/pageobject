...
def test_wait_until_interactive_returns_self_when_all_is_fine(monkeypatch, mock_po):
    mock_po.wait_until = lambda func, func_kwargs={}, timeout=None, error_msg='': True
    assert mock_po.wait_until_interactive() == mock_po

def test_wait_until_interactive_passes_correct_function_to_wait_until(monkeypatch, mock_po):
    ...
    mock_po.wait_until_interactive()
