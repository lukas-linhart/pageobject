...
def test_wait_until_enabled_returns_self_when_all_is_fine(monkeypatch, mock_po):
    ...
    assert mock_po.wait_until_enabled() == mock_po

def test_wait_until_enabled_passes_correct_function_to_wait_until(monkeypatch, mock_po):
    monkeypatch.setattr(mock_po.__class__, '_locator_value', 'locator')
