def wait_for_visible(self, timeout=None):
    ...
    self.logger.info('waiting until page object {} is visible'.format(self._log_id_short))
    self.logger.debug('waiting until page object is visible; {}'.format(self._log_id_long))
    error_msg = 'page object still not visible after {} seconds; {}'.format(
        timeout, self._log_id_long)
    self.wait_until(self.is_visible, func_kwargs=dict(log=False),
        timeout=timeout, error_msg=error_msg)
    self.logger.info('finished waiting until page object {} is visible'.format(self._log_id_short))
