def wait_for_enabled(self, timeout=None):
    ...
    self.logger.info('waiting until page object {} is enabled'.format(self._log_id_short))
    self.logger.debug('waiting until page object is enabled; {}'.format(self._log_id_long))
    error_msg = 'page object still not enabled after {} seconds; {}'.format(
        timeout, self._log_id_long)
    self.wait_until(self.is_enabled, func_kwargs=dict(log=False),
        timeout=timeout, error_msg=error_msg)
    self.logger.info('finished waiting until page object {} is enabled'.format(self._log_id_short))
